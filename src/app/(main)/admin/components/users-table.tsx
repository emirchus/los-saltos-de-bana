...
export function UsersTable({ initialUsers, channel }: UsersTableProps) {
  ...
  const [editedUser, setEditedUser] = useState<{ points: number; stars: number; is_og: boolean } | null>(null);
  ...
  const handleEdit = (user: UserStats) => {
    ...
    setEditedUser({ points: user.points, stars: user.stars, is_og: user.is_og });
  };
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="p-2 text-left text-sm font-medium">OG</th>
                ...
                <tbody>
                  {users.map(user => {
                    ...
                    const currentIsOg = isEditing && editedUser ? editedUser.is_og : user.is_og;
                        ...
                        <td className="p-2">
                          {isEditing ? (
                            <Switch
                              id="is_og"
                              checked={currentIsOg}
                              onCheckedChange={checked => setEditedUser({ ...editedUser!, is_og: checked })}
                              disabled={isSaving}
                            />
                          ) : (
                            <span>{user.is_og ? 'Si' : 'No'}</span>
                          )}
                        </td>
                        <td className="p-2">
                          {isEditing ? (
