...
interface GlobalRankClientProps {
  initialData: Database['public']['Tables']['user_stats']['Row'][];
}

export const GlobalRankClient = ({ initialData }: GlobalRankClientProps) => {
  const searchParams = useSearchParams();
  const page = Number(searchParams.get('page')) || 0;
  const pageSize = Number(searchParams.get('pageSize')) || 100;

  const { data, isLoading, error } = useGlobalRank(page, pageSize, { initialData });
    ...
    >
              ...
              >
                <AnimatePresence>
                  {displayPosition === 1 && (
                    ...
                    >
                      <Star className="w-12 h-12 text-yellow-400 fill-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.8)]" />
                    </motion.div>
                  )}
                </AnimatePresence>
                <motion.div
      ...
      >
        <div className="grid grid-cols-[80px_1fr_120px] gap-4 px-8 py-5 bg-zinc-800/80 text-zinc-400 text-sm font-black uppercase border-b border-amber-500/20">
          <div>#</div>
          <div>Usuario</div>
          <div className="text-right">Estrellas</div>
        </div>
