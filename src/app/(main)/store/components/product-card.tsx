...
export function ProductCard({ product, index }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'group relative bg-card rounded-xl overflow-hidden border border-border hover:border-primary/60 transition-colors',
        {
          'opacity-50': !hasStock,
    ...
    >
      <div className="relative aspect-square p-4 flex items-center justify-center bg-linear-to-b from-secondary/50 to-transparent">
        ...
        <Image
          ...
          width={1500}
          height={1500}
          className="object-contain relative z-10 group-hover:scale-105 transition-transform duration-300 rounded-md"
        />
      </div>
      ...
      <div className="p-4 space-y-2">
        ...
        <p className="text-foreground/80 text-sm line-clamp-2">{product.name}</p>
      </div>
